"""Stage B: signed surface deviation between co-registered sources.

For each configured pair (moving, reference) we compute the signed
point-to-surface distance of the moving cloud against the reference surface.
The sign comes from the reference surface normal (positive = outside / away
from material).
...
signed_deviation(moving_pcd, ref_pcd) returns (distances, stats).
deviations_for_part(part_id, aligned) returns a list of stat dicts.
"""
