"""Main orchestrator for the overhang-specimen distortion pipeline.

It brings three representations of each U-channel overhang specimen (Nominal
CAD, CT surface mesh, Zephyr photogrammetry cloud) into a common leg-based
frame, computes surface deviations, and measures the per-leg distortion angles
and the overhang length. See README.md and the reference slides
(Data Set/Leg and Overhang/Slide1.JPG, Slide2.JPG).

The work is split into phases, each of which can also be run on its own:
    phase0_match      discover and match files into parts (by ID regex)
    phase1_normalize  load and unit/scale normalization
    ...
    phase5_measure    Stage C 4-5: distortion angles and overhang length
    phase6_report     aggregate CSVs, segment clouds, annotated geometry plots

This file loops over the discovered parts and chains the phases, catching
per-part errors so one bad part does not stop the batch.
...
import os
# open3d's RANSAC (global registration and plane fitting) is multithreaded and
# not reproducible across threads even when its RNG is seeded, so we pin OpenMP
# to a single thread to keep the whole pipeline deterministic. numpy/MKL use a
# separate thread pool, so the streaming CT sampler is unaffected. This must be
# set before open3d is imported (via common below).
...
def _visualize_pairs(part_id, aligned):
    """Interactive windows per pair: Nominal vs CT and Nominal vs Zephyr.

    For each pair, two windows open in sequence (close one to advance):
      1. overlay: Nominal (grey) plus the as-built (coloured), to eyeball
         sag/offset.
      2. heat-map: the as-built coloured by signed deviation from Nominal
         (blue = under/inside, white ~ 0, red = proud/outside), clipped to
         +/-config.HEATMAP_CLIP mm.
    """
    import open3d as o3d
...
def process_part(part_id, srcs, args):
    ...
    for src in (s for s in C.SOURCES if s in aligned):
        try:
            ...
            # slot (closing) for both legs. slot_closure is the inter-wall angle
            # vs nominal, which is registration-independent and so valid for
            # Zephyr too.
            if base:
            ...
            # reconstructs the two inner walls unequally; we keep its raw values
            # but flag them low-confidence (trust slot_closure). CT and nominal
            # see both walls, and per-leg lean is registration-invariant, so CT's
            # lower ICP fitness (from genuine part distortion) does not make it
            # unreliable.
            ...
            # leg-vs-span angle expressed as 90 + magnitude of deflection, so it
            # always reads >= 90 (e.g. 91.2, 92.6) regardless of lean direction.
            # Signed in/out direction stays in distortion_leg1/2_deg + slot_closure_deg.
...
def main():
    ...
    # each run would perturb every downstream alignment and metric.
    import open3d as o3d
