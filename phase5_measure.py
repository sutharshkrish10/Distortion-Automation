...
Public API:
    measure_distortion_and_span(segmentation) -> dict
    measure_part(part_id, aligned, nominal_measure=None) -> list[row dict]
"""
...
def _span(seg) -> float:
    ...
    if p1 is None or p2 is None:
        return float("nan")
    # project each inner-wall centroid onto W; their separation is the span,
    # evaluated at the overhang level by construction (walls bound the slot).
...
def _leg_lean(pts, V_dir, W_dir) -> float:
    """Lean of an inner wall measured from its own base: fit the wall's slot-width
    position W as a straight line in height V and return atan(dW/dV) in degrees
    (0 = perfectly upright). Intrinsic to the cloud -- no registration to nominal
    -- and the line fit averages many points, so the slope is far more precise
    than the per-point scatter (SE ~0.03 deg here). The caller subtracts the
    nominal lean to get the leg's distortion (deflection from base)."""
...
def measure_distortion_and_span(seg) -> dict:
    ...
    its base; the caller subtracts the nominal lean) -- registration-free and
    precise. slot_closure (inter-wall angle vs nominal) is also registration-free
    and is the only per-distortion metric trustworthy for Zephyr. The legacy
    overhang-vs-wall angle is returned too, but NaN when the overhang ceiling
    wasn't captured (see _overhang_is_horizontal)."""
...
def measure_part(part_id, aligned, nominal_measure=None):
    ...
    for src in order:
        ...
        # absolute interior leg-vs-span angle (90 nominal, >90 = leg leaned outward)
