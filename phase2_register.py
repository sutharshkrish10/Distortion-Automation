"""Stage A: bring each as-built source into the Nominal frame.

Steps per source (Nominal is the fixed reference):
    3. coarse: voxel-downsample, FPFH features, RANSAC global registration
...
register(source_pcd, target_pcd, voxel_size) returns a RegResult.
register_part(part_id, loaded) returns {source: Aligned} and saves outputs.
"""
...
def _coverage(ref_pts: np.ndarray, aligned_pts: np.ndarray, tol: float) -> float:
    ...
    within `tol`. This is a flip-safe registration score: a wrong basin leaves
    much of the nominal surface uncovered even when its inlier rmse looks fine."""
...
class Aligned:
    source: str
    transform: np.ndarray                 # 4x4 mapping source into nominal frame
...
def register(source_pcd, target_pcd, voxel_size) -> tuple[RegResult, RegResult]:
    """Multi-start coarse RANSAC plus point-to-plane ICP, kept by best coverage.
    ...
    wins (see _coverage). The fixed seed list keeps it deterministic. Returns the
    winning (coarse, fine) RegResults; transforms map the source into the target
    frame."""
...
def _build_axis(pts: np.ndarray, ref: np.ndarray) -> np.ndarray:
    """The part's build (vertical) direction, taken as the PCA eigenvector
    closest to `ref` (the bbox-based world build axis from detect_axes, which is
    robust to a partial or biased cloud). Picking the globally-longest
    eigenvector instead fails on the wider 4mm/6mm parts (build is not the
    longest extent) and on partial Zephyr clouds (PCA is weighted by which faces
    were captured), which sent the correction off by tens of degrees. The
    eigenvector nearest the known build axis is the small residual roll we
...
def _upright_refine(aligned_pcd, nominal_pcd, init_T) -> tuple[np.ndarray, float]:
    ...
    (UPRIGHT_MAX_ROLL_DEG < roll <= UPRIGHT_MAX_APPLY_DEG):
      1. rotate the build axis (the PCA eigenvector nearest the bbox vertical)
         onto world vertical, snapping the part upright; this is the tilt seen in
         the V-W plots.
      2. about that axis, rotate the segmented wall-centroid vector (leg1 to
         leg2) onto world W, so the legs sit left/right like the nominal.
    The magnitude cap rejects the large bogus rolls PCA produces on sparse or
    ...
    c = pts.mean(0)
    R1 = _min_rot(b, V_world)                       # snap build axis to world vertical
    # step 2: about the vertical, rotate the leg1-to-leg2 wall vector onto world
    # W, so the legs sit left/right like the nominal. Skipped if it would be a
    # large (flip-like) rotation, since registration already fixes the gross
    # left/right.
    R2 = np.eye(3)
    try:
...
def _gentle_icp(source_pcd, target_pcd, init_T) -> tuple[np.ndarray, float, float]:
    """Full-cloud point-to-plane ICP refine seeded from init_T. Returns
    (refined_T, delta_rotation_deg, fitness). delta is how far it moved from
    init_T, used to reject anything but a gentle correction. Uses a wider
    correspondence window (ZEPHYR_FINAL_ICP_DIST_MM) than the fine ICP so a few-
    degree tilt (top displaced ~0.8 mm) is actually captured."""
    icp = o3d.pipelines.registration.registration_icp(
        source_pcd, target_pcd, C.ZEPHYR_FINAL_ICP_DIST_MM, init_T,
        o3d.pipelines.registration.TransformationEstimationPointToPlane(),
        o3d.pipelines.registration.ICPConvergenceCriteria(max_iteration=C.ICP_MAX_ITER))
    T = np.asarray(icp.transformation)
    dR = T[:3, :3] @ np.linalg.inv(init_T[:3, :3])
    ang = float(np.degrees(np.arccos(np.clip((np.trace(dR) - 1) / 2, -1.0, 1.0))))
    return T, ang, icp.fitness
    ...
    for src in ("ct", "zephyr"):
        ...
        try:
            ...
            if C.UPRIGHT_REFINE:
                ...
                if roll > 0.0:
                    # Validate: keep the de-roll only if it does not pull the body
                    # off nominal. A bogus PCA roll (4mm Zephyr) tilts the part and
                    # drops coverage; a real residual roll raises (or holds) it.
                    src_pts = np.asarray(ld.cloud.points)
                    ref_pts = np.asarray(nominal.cloud.points)
                    cov_before = _coverage(
                        ref_pts, src_pts @ best.transform[:3, :3].T + best.transform[:3, 3],
                        C.REG_COVERAGE_TOL)
                    cov_after = _coverage(
                        ref_pts, src_pts @ up_T[:3, :3].T + up_T[:3, 3],
                        C.REG_COVERAGE_TOL)
                    if cov_after - cov_before >= C.UPRIGHT_MIN_COVERAGE_GAIN:
                        best = RegResult(up_T, best.fitness, best.inlier_rmse,
                                         best.stage + "+upright")
                    else:
                        LOG.info("[%s] %-6s upright -%.1fdeg REJECTED "
                                 "(coverage %.3f -> %.3f)", part_id, src, roll,
                                 cov_before, cov_after)
                        roll = 0.0
            # Gentle final ICP (Zephyr only): pull out the small residual tilt a
            # partial cloud carries after the leg datum. Kept only if it is a
            # small move and does not reduce nominal coverage, so it can never
            # become a large bogus de-roll.
            gentle = 0.0
            if src == "zephyr" and C.ZEPHYR_FINAL_ICP:
                ref_pts = np.asarray(nominal.cloud.points)
                src_pts = np.asarray(ld.cloud.points)
                ref_T, ang, fit = _gentle_icp(ld.cloud, nominal.cloud, best.transform)
                cov_before = _coverage(
                    ref_pts, src_pts @ best.transform[:3, :3].T + best.transform[:3, 3],
                    C.REG_COVERAGE_TOL)
                cov_after = _coverage(
                    ref_pts, src_pts @ ref_T[:3, :3].T + ref_T[:3, 3],
                    C.REG_COVERAGE_TOL)
                if (ang <= C.ZEPHYR_FINAL_ICP_MAX_DEG
                        and cov_after - cov_before >= C.ZEPHYR_FINAL_ICP_MIN_COVERAGE_GAIN):
                    best = RegResult(ref_T, fit, best.inlier_rmse, best.stage + "+icp")
                    gentle = ang
                elif ang > C.ZEPHYR_FINAL_ICP_MAX_DEG:
                    LOG.info("[%s] %-6s gentle ICP %.1fdeg REJECTED (> %.1f cap)",
                             part_id, src, ang, C.ZEPHYR_FINAL_ICP_MAX_DEG)
                else:
                    LOG.info("[%s] %-6s gentle ICP %.1fdeg REJECTED "
                             "(coverage %.3f -> %.3f)", part_id, src, ang,
                             cov_before, cov_after)
            LOG.info("[%s] %-6s coarse fit=%.3f rmse=%.3f | fine fit=%.3f rmse=%.3f"
                     "%s%s%s", part_id, src, coarse.fitness, coarse.inlier_rmse,
                     fine.fitness, fine.inlier_rmse,
                     f" | leg fit={leg.fitness:.3f} rmse={leg.inlier_rmse:.3f}"
                     if leg else "",
                     f" | upright -{roll:.1f}deg" if roll > 0.0 else "",
                     f" | gentle icp {gentle:.1f}deg" if gentle > 0.0 else "")
            ...
            # the saved .ply is display-only (measurement uses the in-memory
            # Aligned.cloud below). If a stamp-retaining full cloud exists, save
            # THAT so overlays show the stamp; the trimmed cloud still drives
            # segmentation + deviation.
            save_ply = aligned_cloud
            if ld.cloud_full is not None:
                save_ply = copy.deepcopy(ld.cloud_full).transform(best.transform)
            save_cloud(save_ply, out_dir / f"aligned_{src}.ply")
