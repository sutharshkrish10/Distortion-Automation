"""Discover the source files and match them into parts by ID.

Filenames are inconsistent across the three source folders (e.g. "2mm Dense
point cloud.ply", "Mesh from 2PR.stl", "2PR.stl"), so we match on the part size
embedded in the name using a regex. New parts can be added without code changes.

match_parts() returns {part_id: {"nominal": Path|None, "ct": Path|None,
"zephyr": Path|None}}.
"""
...
def _scan(folder: Path, exts: tuple[str, ...]) -> dict[str, Path]:
    """Map part_id to file path for one source folder."""
    found: dict[str, Path] = {}
...
def match_parts() -> dict[str, dict[str, Path | None]]:
    ...
    zephyr  = _scan(C.SOURCE_DIRS["zephyr"],  (".stl", ".ply"))
