...
This operates in the aligned Nominal frame (everything is co-registered there),
so the world axes coincide with the part axes; PCA is used only as a fallback
when the bbox extents are ambiguous.

Geometry, established from the nominal solids:
    - vertical (build) axis = the largest bbox extent.
    - width axis (Leg1 to Leg2) = the horizontal axis whose end-slab splits into
...
segment_legs_overhang(geometry) returns a Segmentation.
"""

from __future__ import annotations

import os
# sklearn/joblib core-detection can fail on some Windows setups (slow subprocess
# and a warning), so we pin it before sklearn is imported.
...
class Segmentation:
    ...
    W_dir: np.ndarray                        # slot-width axis Leg1 to Leg2 (unit)
    ...
    masks: dict = field(default_factory=dict)   # label to bool mask into points
...
def _slot_gap(coords: np.ndarray):
    """KMeans(2) split of 1-D coords into (gap, centre, halfwidth) for a slot.

    gap is the clear distance between the inner edges of the two clusters
...
def detect_axes(points: np.ndarray):
    ...
    # Overhang level along V: the notch ceiling, i.e. the horizontal surface
    # that bridges the legs at the inner end of the slot. Inside the slot (narrow
    # W band) the only solid faces are the ceiling and the part's far outer face;
...
def _fit_plane_pca(pts: np.ndarray, expect_normal: np.ndarray | None = None) -> Plane | None:
    """Least-squares (PCA) plane through `pts`: the normal is the
    smallest-variance axis. Used for the inner walls, where the slab is
    overwhelmingly the wall itself, so PCA is stable and captures the wall's
    lean. RANSAC on such a thin slab can instead latch onto a competing sub-plane
    (e.g. a depth end-face) when inliers are few (seen on 4P2)."""
    if len(pts) < max(C.PLANE_RANSAC_N, 10):
        return None
    c = pts.mean(0)
...
def inner_face_positions(seg):
    """W-coordinates of the two inner-wall faces, i.e. the slot-facing edges of
    each leg's inner band. Leg 1 (lower-W leg) faces the slot on its high-W side
    and Leg 2 (higher-W leg) on its low-W side, so we take a robust top/bottom
    percentile of each band's W. Their gap is the true slot width (nominal parts
    recover their 2 / 4 / 6 mm design slot). Returns (w_leg1_face, w_leg2_face),
    or (None, None) if a band is missing or too sparse."""
    w1 = seg.masks.get("_inner1_pts")
    w2 = seg.masks.get("_inner2_pts")
    if w1 is None or w2 is None or len(w1) < 10 or len(w2) < 10:
        return None, None
    a1 = np.asarray(w1) @ seg.W_dir
    a2 = np.asarray(w2) @ seg.W_dir
    f1 = float(np.percentile(a1, C.SPAN_FACE_PCT))          # leg1 face = high-W edge
    f2 = float(np.percentile(a2, 100.0 - C.SPAN_FACE_PCT))  # leg2 face = low-W edge
    return f1, f2
